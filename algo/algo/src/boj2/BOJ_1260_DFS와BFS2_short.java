 ...
 * 인접행렬
 * */
public class BOJ_1260_DFS와BFS2_short {

	static int[][] matrix; //이차원배열 -> 방문할 수 있는 정점 관계
	//static boolean[] visited; //=> visited 안쓰고 matrix썼을 때 공간복잡도 줄여보기
	...
	static void bfs() {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 1; i <= N; i++) {
				if (matrix[front][i]== 0|| matrix[front][0] == 1) {
					matrix[i][0] = 1;
					queue.offer(i);
				}
			}
	...
	static void dfs(int i) {
		...
		for (int x = 1; x <= N; x++) { //i에서 갈 수 있는 자식 노드 방문
			...
			if (matrix[i][x] != 1 || matrix[i][0]==1) continue;
			matrix[i][0] = 1; //방문했다 즉, 0은 원래 dummy로 썼다. 근데 0번째가 1이면, 방문했다는 의미
			dfs(x);
		}
	}
