...
public class BOJ_2023신기한소수_prof2 {
	...
	static void search(int idx, int num) {
		...
		//1
		int nextNum = nextNum(num, 1); //int to String
		if (isPrime(nextNum)) search(idx+1, nextNum);
		//3
		nextNum = nextNum(num, 3); //int to String
		if (isPrime(nextNum)) search(idx+1, nextNum);
		//7
		nextNum = nextNum(num, 7); //int to String
		if (isPrime(nextNum)) search(idx+1, nextNum);
		//9
		nextNum = nextNum(num, 9); //int to String
		if (isPrime(nextNum)) search(idx+1, nextNum);
	}
